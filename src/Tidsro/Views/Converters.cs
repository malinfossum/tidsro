...
///
/// <para>A multi-value converter because the answer needs three things — the band's blocks, whether
/// its column is today, and the current minute — and only the last of those changes while the window
/// is open. It takes every block in the band rather than one: a block that ends where the next
/// begins shares the band and the lane with it, and the single bar has to light for whichever of
/// them is running. It carries no rule of its own: the decision is
/// <see cref="TimetableLayout.IsCurrent(IEnumerable{TimetableEntry}, bool, int)"/>, which lives in
/// the model where tests reach it. The brushes stay in XAML, keyed off the string this returns.</para></summary>
public sealed class LaneBarStateConverter : IMultiValueConverter
{
    public object Convert(object[] values, Type t, object? p, CultureInfo c)
    {
        if (values is not [IReadOnlyList<TimetableEntry> bars, bool isToday, int nowMinuteOfDay]
            || bars.Count == 0) return "None";
        return TimetableLayout.IsCurrent(bars, isToday, nowMinuteOfDay) ? "Now" : "Block";
    }
